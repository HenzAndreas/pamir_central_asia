...
def initialize(cfg, state):
    print("Use of my custom glacial index process")
    ...
    state.glacial_index = tf.Variable(0.0, dtype="float32")
...
def update(cfg, state):
    if (state.t - state.tlast_clim) >= cfg.processes.clim_glacialindex.update_freq:
        ...
        # assign s to a state for saving
        state.glacial_index.assign(s)
